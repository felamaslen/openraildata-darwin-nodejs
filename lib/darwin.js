...
class Darwin extends EventEmitter {
  ...
  connect(queue) {
    ...
    const servers = new stompit.ConnectFailover([serverConfig(this.clientId)], reconnectOptions);
     ...
     */
    this._channel.on('idle', () => { this.emit('idle'); });
    /**
     * @event Darwin#error
